...
describe("API", () => {
	...
	it("returns a Promise", () => {
		mdeps(__dirname + "/cjs/a.js").should.be.Promise(); // eslint-disable-line new-cap
	});

	it("throws error if file or directory does not exists", (done) => {
		mdeps(__dirname + "/missing.js")
			.catch((err) => {
	...
	it("takes an array of files as path and combines the result", (done) => {
		mdeps([__dirname + "/cjs/a.js", __dirname + "/cjs/normal/d.js"])
			.then((res) => {
				res.obj().should.eql({
					"a.js": ["b.js", "c.js"],
					"b.js": ["c.js"],
	...
	it("takes an array of directories as path and compute the basedir correctly", (done) => {
		mdeps([__dirname + "/cjs/multibase/1", __dirname + "/cjs/multibase/2"])
			.then((res) => {
				res.obj().should.eql({
					"1/a.js": [],
					"2/b.js": [],
	...
	describe("circular()", () => {
		it("returns list of circular dependencies", (done) => {
			mdeps(__dirname + "/cjs/circular/a.js")
				.then((res) => {
	...
	describe("warnings()", () => {
		it("returns an array of skipped files", (done) => {
			mdeps(__dirname + "/cjs/missing.js")
				.then((res) => {
					res.obj().should.eql({
						"missing.js": ["c.js"],
						"c.js": [],
	...
	describe("dot()", () => {
		it("returns a promise resolved with graphviz DOT output", async () => {
			const res = await mdeps(__dirname + "/cjs/b.js");
	...
	describe("image()", () => {
		...
		beforeEach(() => {
			imagePath = path.join(os.tmpdir(), "mdeps_" + Date.now() + "_image.png");
		});
		...
		it("rejects if graphviz is not installed", (done) => {
			mdeps(__dirname + "/cjs/a.js", { graphVizPath: "/invalid/path" })
		...
		it("writes image to file", (done) => {
			mdeps(__dirname + "/cjs/a.js")
				...
				.then((writtenImagePath) => {
					...
					return fs.exists(imagePath).then((exists) => {
						if (!exists) {
