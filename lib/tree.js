...
// const realDependencyTree = require("dependency-tree");
const log = require("./log");

const stat = promisify(require("fs").stat);

// Q-hack
const dependencyTree = (args) => {
	const js = JSON.parse(fs.readFileSync(args.filename, "utf-8"));
	const svcs = js.services.map((x) => x.id);
	const tree = { [js.endpointServiceId]: svcs };
	return tree;
};
...
class Tree {
				...
				return stat(srcPath).then((stats) => {
					...
					walk.sync(srcPath, (filePath, stat) => {
						...
						if (
							...
							this.config.fileExtensions.indexOf(ext) >= 0
						) {
							files.push(filePath);
						}
					});
	...
	generateTree(files) {
				...
				dependencyTree({
					...
					filter: (dependencyFilePath, traversedFilePath) => {
						...
						if (this.config.includeNpm && isNpmPath) {
							(npmPaths[traversedFilePath] =
								npmPaths[traversedFilePath] || []).push(dependencyFilePath);
						}
		...
		let tree = depTree; // this.convertTree(depTree, {}, pathCache, npmPaths);
	 ...
	 * shallow (one level deep) tree used by mdeps.
